...
from .forms import AppointPrimaryAuthorityForm, InteractionForm
from .models import (
    Interaction,
...
def is_manager(user):
    """
    Managers are superusers OR members of the 'Managers' group.
    """
    return (
...
def create_interaction(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            interaction.save()
            # Correct URL name from your urls.py
            return redirect("qms_interaction_create")
    else:
...
def update_interaction(request, pk):
    ...
    if request.method == "POST":
        ...
        if new_status == "closed" and not manager_notes:
            messages.error(
                request, "Manager notes are required before closing an interaction."
            )
            return redirect("qms_manager_list")
        ...
        if previous_assignee != new_assignee:
            InteractionAssignmentLog.objects.create(
                ...
                reason=(reassignment_reason if previous_assignee else "Initial assignment"),
            )
        ...
        if manager_notes:
            ...
            # Optional: if you store notes elsewhere, adjust as needed
        ...
        if (
            email_required
            and interaction.assigned_to
            and getattr(interaction.assigned_to, "email", None)
        ):
            context = {
                "interaction": interaction,
            ...
            text_body = render_to_string("qms/emails/interaction_assigned.txt", context)
            html_body = render_to_string("qms/emails/interaction_assigned.html", context)
...
def investigation_dashboard(request):
    ...
    investigations = Investigation.objects.select_related("staff_member").all()
...
def investigation_my_list(request):
    investigations = Investigation.objects.filter(staff_member=request.user).order_by(
        "-created_at"
    )
...
def investigation_staff_detail(request, pk):
    investigation = get_object_or_404(
        Investigation, pk=pk, staff_member=request.user
    )
...
def investigation_add_log(request, pk):
    ...
    if request.method == "POST":
        ...
        if notes:
            ...
            messages.success(request, "Evidence added to investigation audit log.")
        else:
            messages.warning(request, "No evidence was added. Please enter notes.")

    return redirect("investigation_detail_manager", pk=investigation.pk)


# ─────────────────────────────────────────────────────
# PRIMARY QMS AUTHORITY (SESSION CONFIRM)
# ─────────────────────────────────────────────────────
@login_required
def confirm_primary_authority(request):
    if request.method == "POST":
        ...
        messages.success(request, "Primary QMS Authority confirmed for this session.")
...
def primary_authority_list(request):
    # Fixed key name to match confirm_primary_authority()
    if not request.session.get("primary_qms_confirmed"):
        return redirect("qms_confirm_primary")

    # Fixed ordering field (QMSAuthority has appointed_at, not created_at)
...
def qms_primary_list(request):
    ...
    authorities = QMSAuthority.objects.select_related("user", "appointed_by").order_by(
        "-appointed_at"
    )
